...
class UnoGame:
    ...
    >>> game = UnoGame(5)
    """
    def __init__(self, player_ids, random=True):
        if not 2 <= len(player_ids) <= 15:
            raise ValueError('Invalid game: must be between 2 and 15 players')
        self.deck = self._create_deck(random)
        self.players= []
        for id in player_ids:
            self.players.append(UnoPlayer(self._deal_hand(), id))
    ...
    def play(self, player_id, card=None, new_color=None):
        """
